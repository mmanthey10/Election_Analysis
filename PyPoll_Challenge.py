...
with open(file_to_save, "w") as txt_file:
    ...
    for county_name in county_list:
        ...
        county_voting = county_votes[county_name]
        # 6c: Calculate the percentage of votes for the county.
        vote_percentage = float(county_voting) / float(total_votes) *100

        # 6d: Print the county results to the terminal.
        county_results = (
            f"\n{county_name}: {vote_percentage:.1f}% ({county_voting:,})\n")
        print(county_results)
        # 6e: Save the county votes to a text file.
        txt_file.write(county_results)

        # 6f: Write an if statement to determine the winning county and get its vote count.
        if county_voting > high_turnout_count:
            high_turnout_count = county_voting
            county_high_turnout = county_name

    # 7: Print the county with the largest turnout to the terminal.
    winning_county_results = (
        f"-------------------------------------------------------------------------\n"
        f"\nLargest County Turnout: {county_high_turnout} ({high_turnout_count:,}) \n"
        f"-------------------------------------------------------------------------\n")
